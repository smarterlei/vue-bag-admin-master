...
import { userUserinfo, webLeaveAll, webLeaveAdd } from '@/bag-web/service/app'
...
webLeaveAll({ articleId: route.params.id }).then((res: any) => {
    let messageOnline = res.map((item: any) => {
        return {
            id: item.id, parentId: null, likes: item.likes, uid: item.id, address: item.address, content: item.content, createTime: item.createTime,
            user: {
...
const submit = ({ content, parentId, files, finish, reply }: SubmitParamApi) => {
    ...
    webLeaveAdd({ content, username: config.user.username, userhead, articleId: route.params.id }).then(res => {
